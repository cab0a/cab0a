...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解をPython解析器と形状計算へ接続。</strong>34件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.34.0では、外周と穴を持つ平面枠、その反転面、全周円柱面、球面を解析式から生成し、支持曲面と切り取り領域、外周と内周、辺の接続順、面の向き、周期的な継ぎ目、極の縮退辺を分離しました。平面枠では面積`42`を保ったまま、面反転で外周・内周の符号付き媒介変数面積が`+48/-6`から`-48/+6`へ反転しました。球面では同じ継ぎ目辺を2回使い、三次元曲線を持たない縮退辺2本が二次元境界を閉じることをSTEP再読込後にも確認しました。これは固定した合成形状の回帰結果であり、任意の曲線境界、壊れた位相、Bスプライン、一般的なSTEP適合、第三者バイナリの再配布許可は主張しません。公開版は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
