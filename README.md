...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解をPython解析器と形状計算へ接続。</strong>v0.40.0まで40件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.36〜v0.38では許容差・縫合・修復、多様体性・自己干渉、空洞・内殻・複数立体を検証しました。v0.39ではSTEP再読込時に23面と47辺を一対一対応させ、区別不能な2面と8辺では棄権し、修復時の10面から6面、20辺から12辺への統合・削除を処理履歴と照合しました。v0.40では9合成形状から穴、段差、溝、面取り状、丸み状の幾何候補を抽出し、構築時とSTEP再読込時の14/14候補で分類・寸法が既知の真値に一致しました。負例の誤検出は0件で、操作による面取りと同等な直接斜面は位相・体積が一致し、双方向差分体積も0でした。幾何候補は設計意図や製造工程を証明せず、曲線形状、特徴同士の相互作用、一般的なCADモデルは未対応です。公開版は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
