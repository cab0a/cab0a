...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解をPython解析器と形状計算へ接続。</strong>v0.42.0まで42件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.39ではSTEP再読込と修復をまたぐ面・辺対応で、幾何、位相による候補支持、処理履歴、直接同一性を分離しました。v0.40では9合成形状から穴、段差、溝、面取り状、丸み状の幾何候補を抽出し、構築時とSTEP再読込時の14/14候補で分類・寸法が既知の真値に一致しました。v0.41では5合成形状を構築時とSTEP再読込後に解析し、各段階の合計13面について、親立体・親殻、6種類の曲面、向き、面積、重心、媒介変数範囲、代表法線、曲面固有値、輪郭線、境界辺、公差、隣接面、名前・色の出典を60列のCSV契約へ統合しました。v0.42では3合成STEP形状を距離条件2水準と角度条件2水準で三角形分割し、3,782個の三角形を解析用面と元のSTEP面へ直接対応付けました。曲面ごとに支配的な条件が異なり、球の極に残る面積ゼロ三角形も明示しています。分割条件や一点標本を最大誤差保証とせず、面番号・三角形番号・元のSTEP面番号は処理内だけで有効です。恒久的な位相同一性、一般的なCADモデル、設計意図、製造工程、保証付き三角形分割誤差は未対応です。公開版は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
