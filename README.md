...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解を再現可能なPythonパーサーへ接続。</strong>23件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.23.0ではv0.21とv0.22のPart 21読み取り処理を一つのsource-preserving parserへ統合し、10個の合成fixtureで元の綴り・空白・コメント・文字位置・UTF-8 byte位置、単純・複合entity、前方参照、構文異常、resource上限を検証します。acceptした5件は元bytesを完全再構成し、閉じた四面体の74 entities・97 referencesと面・辺・シェル・立体の位相解析も維持します。完全なPart 21、EXPRESS・AP242適合、幾何妥当性、書き戻し互換性を未検証のまま成功扱いしません。 |
