...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解をPython解析器と形状計算へ接続。</strong>35件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.35.0では、正常・全体反転・開放・1面反転・非多様体・種数1・非連結の7形状を合成し、辺の使用回数、面の連結成分、閉包、向き付け、オイラー標数、符号付き体積を独立に評価しました。14件の構築時・STEP再読込時観測で既知の位相値を保持しましたが、一般妥当性が真でも開いた外殻、向き不正、非多様体を許す3例を確認しました。STEP往復では全体反転した直方体の体積符号が`-120`から`+120`へ変わり、1面反転が整列し、非多様体と非連結の外殻構成が分割されました。これは固定合成形状と単一形状計算経路の回帰結果であり、頂点近傍の多様体性、自己交差、許容差付き縫合、一般的なSTEP適合、第三者バイナリの再配布許可は主張しません。公開版は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
