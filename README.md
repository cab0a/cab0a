...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解をPython解析器と形状計算・モデリングへ接続。</strong>v0.55.0まで55件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.52では8形状群と4種類の変動を組み合わせた32条件を構築時とSTEP読込後に評価し、各段階で22件を受理、8件を拒否、軸整列前提を外れた2件を判定保留にしました。v0.53では36個の合成STEPを9形状系列単位で学習・調整・試験へ分け、5種類の漏えい検査を違反0件で固定しました。v0.54では限定規則、幾何量、面隣接グラフ、表形式の4方式を比較し、成功だけでなく確信した誤判断も記録しました。v0.55では基準面、寸法、スケッチ、形状操作、結果形状を版番号付き非巡回グラフとして表し、3形状を独立真値とSTEP往復で検証しました。読込STEPの穴候補は未確認のまま生成結果と分離しています。恒久的な位相命名、一般的な形状特徴認識、拘束条件解法、一般再計算、設計意図復元は未対応です。公開版は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
