...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解をPython解析器と形状計算へ接続。</strong>v0.41.0まで41件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.39ではSTEP再読込と修復をまたぐ面・辺対応で、幾何、位相による候補支持、処理履歴、直接同一性を分離しました。v0.40では9合成形状から穴、段差、溝、面取り状、丸み状の幾何候補を抽出し、構築時とSTEP再読込時の14/14候補で分類・寸法が既知の真値に一致しました。v0.41では5合成形状の各13面について、親立体・親殻、平面・円筒・円すい・球・トーラス・Bスプライン、向き、面積、重心、媒介変数範囲、代表法線、曲面固有値、輪郭線、境界辺、公差、隣接面、名前・色の出典を60列のCSV契約へ統合しました。面番号は段階内だけで有効とし、STEP形状から取得できない名前・色は推測しません。恒久的な位相同一性、一般的なCADモデル、設計意図、製造工程は未対応です。公開版は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
