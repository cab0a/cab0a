...
| [Research Notes](https://github.com/cab0a/research-notes) | <strong>STEP仕様理解を再現可能なPythonパーサーへ接続。</strong>27件の研究で文献調査、統制実験、結果、考察、制約を記録しています。v0.27.0ではSTEP Part 21とEXPRESS semantic graphを結合し、DATA部のschema、entity、引数、値、参照、継承を40組の合成fixtureで検証しました。15件を受理、21件を拒否、4件を隔離し、35個の引数を妥当、13個を不適合、2個を保留として属性単位で記録します。これは完全なPart 21・EXPRESS適合、複合entityのevaluated set、rule実行、AP242意味論、幾何評価の主張ではありません。v0.27.0以前はMIT License、以後のmainとv0.28.0以降は研究・教育・個人的実験向けのPolyForm Noncommercial License 1.0.0とし、商用利用は書面による別契約とします。 |
